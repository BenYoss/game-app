...
`;
  try {
    let thread = await pool.query(getThreadCommand, [idThread]);
    thread = thread.rows;
    const replies = await pool.query(getRepliesCommand, [idThread]);
    console.log('This is where', replies.rows);
    thread[0].replies = replies.rows ? replies.rows : [];
    return thread;
  } catch (error) {
    return console.error('COULD NOT GET REPLIES FROM DATABASE', error);
  }
}
